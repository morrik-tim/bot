...
async def select_callback_handler(query: types.CallbackQuery):
    ...
    await button_queue.put('select')


@dp.message_handler(lambda query: query.data == 'next')
async def next_callback_handler():
    await button_queue.put('next')
    print('ti loh')


@dp.message_handler(lambda query: query.data == 'back')
async def back_callback_handler():
    await button_queue.put('back')
    print('ti loh nazad')
...
async def process_film(film, markup, chat_id):
    ...
    if content == 'movie':
        print('видео')
        if player.post.translators.names in player.post.translators.name_id.items() is not None:
